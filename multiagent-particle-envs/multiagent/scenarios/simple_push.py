...
class Scenario(BaseScenario):
    ...
    def adversary_reward(self, agent, world):
        ...
        pos_rew = 5.0 * min(agent_dist)
        nearest_agent = [a for a in world.agents if not a.adversary][0]
        ...
        neg_rew = 0.5 * np.sqrt(np.sum(np.square(nearest_agent.state.p_pos - agent.state.p_pos)))
