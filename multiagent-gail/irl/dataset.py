...
class Dset(object):
    def __init__(self, inputs, labels, all_obs, rews, randomize, num_agents):
    ...
    def update(self, inputs, labels, all_obs, rews, decay_rate=0.9):
        idx = np.arange(self.num_pairs)
        np.random.shuffle(idx)
...
class MADataSet(object):
    def __init__(self, expert_path, train_fraction=0.7, ret_threshold=None, traj_limitation=np.inf, randomize=True):
        ...
        self.randomize = randomize
        self.dset = Dset(self.obs, self.acs, self.all_obs, self.rews, self.randomize, num_agents)
        # for behavior cloning
        self.train_set = Dset(self.obs, self.acs, self.all_obs, self.rews, self.randomize, num_agents)
        self.val_set = Dset(self.obs, self.acs, self.all_obs, self.rews, self.randomize, num_agents)
        self.log_info()
...
if __name__ == '__main__':
    ...
    parser.add_argument("--expert_path", type=str,
                        default="/Users/LantaoYu/PycharmProjects/multiagent-irl/models/mack-simple_tag-checkpoint20000.pkl")
