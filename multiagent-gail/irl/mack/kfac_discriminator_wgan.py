...
class Discriminator(object):
    def __init__(self, sess, ob_spaces, ac_spaces,
        ...
        try:
            nacts = [ac.n for ac in ac_spaces]
            nact = ac_space.n
        except:
            nacts = [ac.shape[0] for ac in ac_spaces]
            nact = ac_space.shape[0]

        self.ac_shape = nact * nstack
        self.all_ob_shape = sum([obs.shape[0] for obs in ob_spaces]) * nstack
        self.all_ac_shape = sum(nacts) * nstack
        ...
        labels = tf.concat([tf.ones([tf.shape(self.g)[0], 1]), -tf.ones([tf.shape(self.e)[0], 1])], axis=0)
        ...
        fisher_loss = -tf.reduce_mean(tf.pow(logits - tf.stop_gradient(sample_net), 2))
    ...
    def build_graph(self, x, num_outputs=1, reuse=False):
        with tf.variable_scope(self.scope):
            ...
            logits = fc(p_h1, 'out', nh=num_outputs, act=lambda x: x)
        return logits
