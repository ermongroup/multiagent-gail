...
from irl.mack.com_discriminator import Discriminator
...
class Runner(object):
    ...
    def run(self):
        ...
        for n in range(self.nsteps):
            #print (len(self.obs), len(self.obs[0]), len(self.obs[0][0]), len(self.obs[1]), len(self.obs[1][0]))
            ...
            if self.disc_type == 'decentralized':
                ...
                rewards = [np.squeeze(self.discriminator[k].get_reward(
                    np.concatenate(self.obs, axis=1), multionehot(self.actions[k], self.n_actions[k])))
                    for k in range(self.num_agents)]
                '''
                ...
                rewards = rewards.swapaxes(1, 0)
            elif self.disc_type == 'competitive_single':
                mul = [multionehot(self.actions[k], self.n_actions[k]) for k in range(self.num_agents)]
                rewards = self.discriminator.get_reward(np.concatenate(self.obs, axis=1), np.concatenate(mul, axis=1))
                #rewards = np.repeat(rewards, self.num_agents).reshape(len(rewards), self.num_agents)
                rewards = np.array([rewards, -1*rewards]).reshape(self.num_agents, len(rewards))
                #rewards = rewards.swapaxes(1, 0)
            else:
                assert False
...
def learn(policy, expert, env, seed, total_timesteps=int(40e6), gamma=0.99, lam=0.95, log_interval=1, nprocs=32,
    ...
    if disc_type == 'decentralized':
        ...
        discriminator = Discriminator(model.sess, ob_space, ac_space, nstack, 0, disc_type=disc_type,
                                      ...
                                      lr_rate=dis_lr)
    elif disc_type == 'competitive_single':
        discriminator = Discriminator(model.sess, ob_space, ac_space, nstack, 0, disc_type=disc_type,
                                      total_steps=total_timesteps // (nprocs * nsteps),
                                      scope='discriminator',
                                      lr_rate=dis_lr)
    ...
    for update in range(1, total_timesteps // nbatch + 1):
        ...
        ge_loss, ee_loss, eg_loss = np.zeros((num_agents, d_iters)), np.zeros((num_agents, d_iters)), np.zeros((num_agents, d_iters))
        idx = 0
        ...
        for d_iter in range(d_iters):
            ee_obs, ee_actions, ee_all_obs, _ = expert.get_next_batch(d_minibatch)
            ge_obs, ge_actions_e, ge_all_obs, _ = expert.get_next_batch(d_minibatch)
            eg_obs, eg_actions_e, eg_all_obs, _ = expert.get_next_batch(d_minibatch)
            pre_actions = [np.zeros((nenvs, )) for _ in range(num_agents)] # random action is OK
            ge_actions_g , eg_actions_g = [], []
            ge_obs_1 = np.array(ge_obs[0])
            ge_obs_2 = np.array(ge_obs[1])
            eg_obs_1 = np.array(eg_obs[0])
            eg_obs_2 = np.array(eg_obs[1])
            for k in range(d_minibatch // nenvs):
                ge_actions_g_t, _, _ = model.step([ge_obs_1[k*nenvs:(k+1)*nenvs,:], ge_obs_2[k*nenvs:(k+1)*nenvs,:]], pre_actions)
                ge_actions_g.append(ge_actions_g_t)
                eg_actions_g_t, _, _ = model.step([eg_obs_1[k*nenvs:(k+1)*nenvs,:], eg_obs_2[k*nenvs:(k+1)*nenvs,:]], pre_actions)
                eg_actions_g.append(eg_actions_g_t)
            ge_actions_g = np.array(ge_actions_g).swapaxes(1,0).reshape(num_agents, d_minibatch)
            eg_actions_g = np.array(eg_actions_g).swapaxes(1,0).reshape(num_agents, d_minibatch)
            mh_ge_actions_g = [multionehot(ge_actions_g[k], env.action_space[k].n) for k in range(num_agents)]
            mh_eg_actions_g = [multionehot(eg_actions_g[k], env.action_space[k].n) for k in range(num_agents)]
            ge_actions = np.array([mh_ge_actions_g[0], ge_actions_e[1]])
            eg_actions = np.array([eg_actions_e[0], mh_eg_actions_g[1]])
            ...
            if disc_type == 'competitive_single':
                eg_loss_t, ee_loss_t, ge_loss_t, _ = discriminator.train(
                    eg_all_obs, 
                    np.concatenate(eg_actions, axis=1), 
                    ee_all_obs, 
                    np.concatenate(ee_actions, axis=1),
                    ge_all_obs, 
                    np.concatenate(ge_actions, axis=1)
                )
                eg_loss[:, d_iter] = eg_loss_t
                ee_loss[:, d_iter] = ee_loss_t
                ge_loss[:, d_iter] = ge_loss_t
            else:
                assert False
            idx += 1
        ...
        if update % log_interval == 0 or update == 1:
            ...
            eg_loss_m = np.mean(eg_loss, axis=1)
            ee_loss_m = np.mean(ee_loss, axis=1)
            ge_loss_m = np.mean(ge_loss, axis=1)

            for k in range(num_agents):
                logger.record_tabular("eg_loss %d" % k, eg_loss_m[k])
                logger.record_tabular("ee_loss %d" % k, ee_loss_m[k])
                logger.record_tabular("ge_loss %d" % k, ge_loss_m[k])
